...
bool ext::opengl::Shader::validate() {
...
#if 0
	{
		...
		for ( auto& buffer : buffers ) {
			if ( !(buffer.usageFlags & uf::renderer::enums::Buffer::UNIFORM) ) continue;
...
ext::opengl::Buffer* ext::opengl::Shader::getUniformBuffer( const std::string& name ) {
	...
	for ( size_t bufferIndex = 0, uniformCounter = 0; bufferIndex < buffers.size(); ++bufferIndex ) {
		if ( !(buffers[bufferIndex].usageFlags & uf::renderer::enums::Buffer::UNIFORM) ) continue;
...
ext::opengl::Buffer* ext::opengl::Shader::getStorageBuffer( const std::string& name ) {
	...
	for ( size_t bufferIndex = 0, storageCounter = 0; bufferIndex < buffers.size(); ++bufferIndex ) {
		if ( !(buffers[bufferIndex].usageFlags & uf::renderer::enums::Buffer::STORAGE) ) continue;
...
void ext::opengl::Pipeline::update( Graphic& graphic, GraphicDescriptor& descriptor ) {
	...
	for ( auto& shader : graphic.material.shaders ) {
		#define PARSE_BUFFER( buffers ) for ( auto& buffer : buffers ) {\
			...
			if ( buffer.usageFlags & uf::renderer::enums::Buffer::STORAGE ) {\
...
*/
	...
	for ( auto& buffer : buffers ) {
		if ( buffer.usageFlags & uf::renderer::enums::Buffer::VERTEX ) vertexBuffer = &buffer;
		if ( buffer.usageFlags & uf::renderer::enums::Buffer::INDEX ) indexBuffer = &buffer;
	}
	...
	//	std::cout << index << "  ";
		...
		if ( vertexAttributeColor.name != "" ) {
			...
			switch ( vertexColorState ) {
				...
				case 1: glVertex.color = {0, 1, 0}; break;
				case 2: glVertex.color = {0, 0, 1}; break;
			}
			if ( ++vertexColorState > 2 ) vertexColorState = 0;
		}
		if ( vertexAttributeUv.name != "" ) {
