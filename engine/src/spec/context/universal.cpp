...
void UF_API_CALL spec::uni::Context::initialize() {
	...
	if (version) {
		...
		this->m_settings.majorVersion = version[0] - '0';
		this->m_settings.minorVersion = version[2] - '0';
	}
	else {
		// Can't get the version number, assume 2.0
		this->m_settings.majorVersion = 2;
		this->m_settings.minorVersion = 0;
	}

	// Enable antialiasing if needed
	if (this->m_settings.antialiasingLevel > 0)
