...
//	this->m_settings = settings;
	...
	int bestFormat = 0;
	if ( glSettings.antialiasingLevel > 0 ) {
		...
		if ( wglChoosePixelFormatARB ) {
			...
			int attributes_ints[] = {
					...
					WGL_SAMPLE_BUFFERS_ARB, 	(glSettings.antialiasingLevel ? GL_TRUE : GL_FALSE),
					WGL_SAMPLES_ARB, 			(int) glSettings.antialiasingLevel,
					0, 							0
				};
			...
			do {
				attributes_ints[11] = glSettings.antialiasingLevel;
			...
			} while ( (!isValid || nbFormats == 0) && glSettings.antialiasingLevel-- > 0 );
	...
	if (bestFormat == 0) {
		...
		descriptor.cColorBits   = (BYTE) glSettings.bitsPerPixel;
		descriptor.cDepthBits   = (BYTE) glSettings.depthBits;
		descriptor.cStencilBits = (BYTE) glSettings.stencilBits;
		descriptor.cAlphaBits   = glSettings.bitsPerPixel == 32 ? 8 : 0;
	...
	glSettings.depthBits   	= actualFormat.cDepthBits;
	...
	while (!this->m_context && (glSettings.majorVersion >= 3)) {
		PFNWGLCREATECONTEXTATTRIBSARBPROC wglCreateContextAttribsARB = reinterpret_cast<PFNWGLCREATECONTEXTATTRIBSARBPROC>(wglGetProcAddress("wglCreateContextAttribsARB"));
		if (wglCreateContextAttribsARB) {
			int attributes[] = {
				WGL_CONTEXT_MAJOR_VERSION_ARB,  (int) glSettings.majorVersion,
				WGL_CONTEXT_MINOR_VERSION_ARB,  (int) glSettings.minorVersion,
		...
		if (!this->m_context) {
			if (glSettings.minorVersion > 0) {
				// If the minor version is not 0, we decrease it and try again
				glSettings.minorVersion--;
			}
			else {
				// If the minor version is 0, we decrease the major version
				glSettings.majorVersion--;
				glSettings.minorVersion = 9;
			}
		}
	}
