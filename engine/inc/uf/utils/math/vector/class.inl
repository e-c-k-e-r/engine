...
template<typename T, std::size_t N> 											// copies data into POD from 'components' (typed as std::vector<T>)
uf::Vector<T,N>::Vector(const std::vector<T>& components) {
...
std::string uf::Vector<T,N>::toString() const {
