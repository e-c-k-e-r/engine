...
namespace ext {
	namespace opengl {
		...
		ext::opengl::enums::Type::type_t typeToEnum() {
			if ( typeid(T) == typeid(int8_t) ) return ext::opengl::enums::Type::BYTE;
			if ( typeid(T) == typeid(uint8_t) ) return ext::opengl::enums::Type::UBYTE;
			if ( typeid(T) == typeid(int16_t) ) return ext::opengl::enums::Type::SHORT;
			if ( typeid(T) == typeid(uint16_t) ) return ext::opengl::enums::Type::USHORT;
			if ( typeid(T) == typeid(int32_t) ) return ext::opengl::enums::Type::INT;
			if ( typeid(T) == typeid(uint32_t) ) return ext::opengl::enums::Type::UINT;
			if ( typeid(T) == typeid(float) ) return ext::opengl::enums::Type::FLOAT;
			if ( typeid(T) == typeid(double) ) return ext::opengl::enums::Type::DOUBLE;
			return 0;
		}
	}
}