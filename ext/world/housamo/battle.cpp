...
//		ext::Housamo* transient = transients.at(uid);
		...
		//	if ( morePower != 0 ) payload["message"] = payload["message"].asString() + "\n"+colorString("FF00FF") + "" + std::to_string((int)(morePower*100)) + "%"+colorString("FF0000") + " power modifier"+colorString("FFFFFF") + "";
			...
			float damage = 0.0f; {
				...
				if ( elementData["type"] == "Physical"  ) {
					if ( skillData["hp%"].isNumeric() ) {
						// max hp * power
					//	skillData["power"] = std::sqrt( skillData["power"].asFloat() ) * 10.0f;
						calcDamage = (member["max hp"].asFloat() * skillData["power"].asFloat() * 0.00114f );
						std::cout << __LINE__ << ": " << member["max hp"].asFloat() << " * " << skillData["power"].asFloat() << " * 0.0114f = " << calcDamage << std::endl;
					} else {
						// ( lv + str ) * power / 15
						calcDamage = (member["lv"].asFloat() + member["str"].asFloat()) * skillData["power"].asFloat() / 15.0f;
						std::cout << __LINE__ << ": " << member["lv"].asFloat() << " + " << member["str"].asFloat() << " * " << skillData["power"].asFloat() << " / 15.0f = " << calcDamage << std::endl;
					}
					...
					if ( member["lv"].asFloat() < 30 ) {
						...
						std::cout << __LINE__ << ": " << skillData["power"].asFloat() << " * " << member["lv"].asFloat() << " * ( 2 * " << member["mag"].asFloat() << " + 70.0f ) / 1000.0f = " << calcDamage << std::endl;
						...
						std::cout << __LINE__ << ": " << "3.0f * " << skillData["power"].asFloat() << " * ( 2 * " << member["mag"].asFloat() << " + 70.0f - ( 0.4f * " << member["lv"].asFloat() << " ) ) / 100.0f = " << calcDamage << std::endl;
					} else {
						calcDamage = 3.0f * skillData["power"].asFloat() * ( 2 * member["mag"].asFloat() + 5 ) / 100.0f;
						std::cout << __LINE__ << ": " << "3.0f * " << skillData["power"].asFloat() << " * ( 2 * " << member["mag"].asFloat() << " + 5 ) / 100.0f = " << calcDamage << std::endl;
					}
				} else {
			...
			std::function<void(const std::string&)> applyDamage = [&]( const std::string& targetId ) {
				...
				bool isCrit = ((rand() % 100) / 100.0) < 0.5;
				...
				if ( elementId == "14" ) {
					...
					damage = -(target["max hp"].asFloat() * power);
				}
	...
	*/
		...
		//	playSound(*this, metadata["battle"]["enemy"]["uid"].asUInt64(), "killed");
			// kill
			{
				...
				uf::Entity*  = world.findByUid(uid);
				if (  ) {
					ext::Object& parent = ->getParent<ext::Object>();
					->destroy();
					parent.removeChild(*);
				}
			}
