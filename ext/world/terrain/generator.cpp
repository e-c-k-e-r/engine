...
namespace {
	...
	ext::TerrainVoxel::light_t AMBIENT_LIGHT = 0xFFF; //{std::numeric_limits<ext::TerrainVoxel::light_t::type_t>::max(), std::numeric_limits<ext::TerrainVoxel::light_t::type_t>::max(), std::numeric_limits<ext::TerrainVoxel::light_t::type_t>::max()};
	struct COLOR {
		uint r : 4;
		uint g : 4;
		uint b : 4;
...
ext::TerrainVoxel::uid_t ext::TerrainGenerator::getVoxel( int x, int y, int z ) const {
	// check oob
	if ( x < 0 || y < 0 || z < 0 || x >= this->m_size.x || y >= this->m_size.y || z >= this->m_size.z ) {
		...
		for ( std::size_t __ = 0; __ < _.length; ++__ ) {
			{
				...
				ext::TerrainVoxel voxel = ext::TerrainVoxel::atlas( _.value );
			//	ext::TerrainVoxel::light_t light = this->getLight( x, y, z );
				...
				struct {
					ext::TerrainVoxel::light_t top, bottom, left, right, front, back;
				} light;
				...
				if ( should.right ) {
					for ( uint i = 0; i < model.position.right.size() / 3; ++i ) {
						...
						//	else std::cout << "BLACK: " << std::hex << (uint32_t) light.right << std::endl;
						}
