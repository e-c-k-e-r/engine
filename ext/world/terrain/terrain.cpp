...
void ext::Terrain::initialize() {
	...
	if ( this->hasComponent<ext::vulkan::RTGraphic>() ) {
		...
		rt.compute.diffuseTexture.loadFromFile( "./data/textures/texture.png", ext::vulkan::device, ext::vulkan::device.graphicsQueue );
	...
	this->addHook( "terrain:Reload.%UID%", [&](const std::string& event)->std::string{	
		...
		newMesh.graphic.bindUniform<uf::MeshDescriptor>();
		...
		newMesh.graphic.texture.loadFromFile( texture, ext::vulkan::device, ext::vulkan::device.graphicsQueue );
		newMesh.graphic.initialize( ext::vulkan::device, ext::vulkan::swapchain );
	...
	/* Update uniforms */ if ( this->hasComponent<ext::TerrainGenerator::mesh_t>() ) {
		...
		if ( false ) {
			for ( std::size_t i = 0; i < 16; ++i ) {
				std::cout << uniforms.matrices.projection[i] << " ";
				if ( (i+1) % 4 == 0 ) std::cout << "\n";
			}
			std::cout << std::endl;
		}
