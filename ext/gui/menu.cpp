...
namespace {
	void playSound( ext::GuiMenu& entity, const std::string& key ) {
		ext::Scene& world = entity.getRootParent<ext::World>();
...
void ext::GuiMenu::initialize() {
	...
	ext::Scene& world = this->getRootParent<ext::Scene>();
	uf::Serializer& masterdata = world.getComponent<uf::Serializer>();
	...
	}
	{
		...
		/* Transient shadow background */ {
			...
			ext::Asset& assetLoader = transientSpriteShadow->getComponent<ext::Asset>();
		...
		/* Transient transientSprite */ {
			...
			ext::Asset& assetLoader = transientSprite->getComponent<ext::Asset>();
	...
	*/
	...
	/*
		if ( uf::Window::isKeyPressed("O") && uf::Window::isKeyPressed("J") ) {
			transform.scale.x += uf::physics::time::delta * -0.5f;
			std::cout << "Scale: " << transform.scale.x << ", " << transform.scale.y << std::endl;
		}
		...
		if ( uf::Window::isKeyPressed("O") && uf::Window::isKeyPressed("I") ) {
			transform.scale.y += uf::physics::time::delta * -0.5f;
			std::cout << "Scale: " << transform.scale.x << ", " << transform.scale.y << std::endl;
		}
		...
		if ( uf::Window::isKeyPressed("J") ) {
			transform.position.x += uf::physics::time::delta * -0.5f;
		...
		if ( uf::Window::isKeyPressed("I") ) {
			transform.position.y += uf::physics::time::delta * -0.5f;
			std::cout << "Position: " << transform.position.x << ", " << transform.position.y << std::endl;
		}
...
void ext::GuiMenu::destroy() {
	uf::Serializer& metadata = this->getComponent<uf::Serializer>();
	/* Play sound effect */ if ( !metadata["system"]["closed"].asBool() ) {
		ext::Scene& world = this->getRootParent<ext::Scene>();
		ext::Asset& assetLoader = world.getComponent<ext::Asset>();
